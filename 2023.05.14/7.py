def to_ten(num: str, int_1: int) -> int :
    ...
    dict_num = dict(zip([str(i) for i in range(0, 10)] + [str(chr(i)) for i in range(97, 123)], [i for i in range(0, 36)]))
...
def to_base(num: int, int_1: int) -> str:
    ...
    while True:
        remeinder = int(num) % int_1
        num = int(num) // int_1       
        num_out.append(remeinder)
        if num < int_1:
            num_out.append(num)
            break
    num_out.reverse()
...
def int_base(num: str, arg1: int, arg2: int) -> str | None:
    ...
    list_num = [str(i) for i in range(0, 10)] + [str(chr(i)) for i in range(97, 123)]
    
    if 2 <= arg1 <= 36 and 2 <= arg2 <= 36 and not ''.join(i for i in num.lower() if i in list_num[arg1:]):
