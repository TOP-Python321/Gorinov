...
def dump_students_list(n: int = 1500):
    students = [
        generate_person(2004, 2007) | {
            ...
            'grade': str(sum(sample([2, 3, 3, 4, 4, 5, 4, 5], k=4)) / 4)
        }
        for _ in range(n)
    ]
...
def dump_teachers_list(n: int = 1500, ret: bool = False):
    teachers = [
...
def dump_administrator_list(n: int = 10):
    administrators = []
    for elem in dump_teachers_list(n, True):        
        del(elem['courses'])        
        administrators.append(elem)        
    
    with open(DATA_DIR / f'administrators.json', 'w', encoding='utf-8') as json_file:
        dump(administrators, json_file, ensure_ascii=False, indent=2)

if __name__ == '__main__':
    load_data()
    dump_students_list(100)
    dump_teachers_list(100)
    dump_administrator_list(15)
